...
void animEvent::processInput(IDEvent a_eventID, ConsoleRE::INPUT_DEVICE a_device)
{
	switch (a_device) 
	{
...
void animEvent::loadIDAnimEventConfig_Mouse(CSimpleIniA& a_ini)
{
	auto readOneLine = [](std::string a_line) 
	{
		...
		ConsoleRE::BSFixedString ae = vec[1];
		...
		{
			auto aeVec = std::make_shared<std::vector<ConsoleRE::BSFixedString>>();
			aeVec->push_back(ae);
