...
{
	xUtilty::Log::GetSingleton()->Write(xUtilty::Log::logLevel::kNone,"loading movement trace spells...");
	CSimpleIniA pluginSettings;
	if (!simpleIniUtils::readSimpleIni(pluginSettings, movementInputTraceSettingsDir)) 
	{
		return;
	}
	auto data = ConsoleRE::TESDataHandler::GetSingleton();
	...
	xUtilty::Log::GetSingleton()->Write(xUtilty::Log::logLevel::kNone,"...done");
}
...
void movementInputTracer::onThumbStickMovement(ConsoleRE::ThumbstickEvent* a_thumbStickMovementInput) 
{
	//DEBUG("x: {}, y: {}", x, y);
	updateThumbstickInputTrace(getThumbStickZone(a_thumbStickMovementInput->xValue, a_thumbStickMovementInput->yValue));
}

void movementInputTracer::onKeyBoardMovement(ConsoleRE::ButtonEvent* a_keyboardMovementInput) 
{
	if (a_keyboardMovementInput->IsDown()) 
	{
		std::string userEvent = a_keyboardMovementInput->QUserEvent();
